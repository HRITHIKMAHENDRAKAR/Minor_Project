    ...
    if st.button("Trigger Neural Inference 🚀"):
        try:
            ...
            with st.spinner("Running main backend pipeline (`main.py`)..."):
                ...
                cmd = [sys.executable, "main.py", "--n_sources", str(n_sources)]
                if enable_location:
            ...
            with st.spinner("Parsing results..."):
                ...
                if len(df) == 0:
                    ...
                    # Filter the dataframe to ONLY include the currently processed file
                    current_base_name = os.path.splitext(uploaded_file.name)[0]
                    df_current = df[df["Filename"] == current_base_name]
                    
                    if len(df_current) == 0:
                        # Fallback just in case the name mismatch happened
                        df_current = df.tail(1)
                        
                    if "Top_Prediction_Confidence" in df_current.columns:
                        df_sorted = df_current.sort_values(by="Top_Prediction_Confidence", ascending=False)
                    else:
                        df_sorted = df_current.sort_values(by="Ensemble_Score", ascending=False)
